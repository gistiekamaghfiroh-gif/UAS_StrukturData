...
from encoder import build_codes, encode
from decoder import decode


def build_huffman_tree(text):
    # Buat tabel untuk menghitung berapa kali setiap karakter muncul
    ht = HashTable()

    # Hitung banyaknya kemunculan setiap karakter dalam teks
    for ch in text:
        ht.insert(ch)

    # Ambil hasil frekuensi karakter
    freq = ht.get_frequency()

    # Buat heap untuk memilih karakter dengan frekuensi paling kecil
    heap = MinHeap()

    # Masukkan setiap karakter ke dalam heap
    for ch, f in freq.items():
        heap.insert(Node(ch, f))

    # Bangun pohon Huffman dengan menggabungkan dua node paling kecil
    while heap.size() > 1:
        left = heap.extract_min()  # node terkecil pertama
        right = heap.extract_min()  # node terkecil kedua

        # Buat parent yang jumlah frekuensinya adalah gabungan keduanya
    ...
    # Root dari pohon Huffman
    root = heap.extract_min()
    return freq, root


def main():
    # Menyimpan semua teks yang sudah di-encode
    data_list = []
